...
public interface JNADll extends Library {
	...
	public static class VZ_LPRC_IMAGE_INFO_Pointer extends Structure {
		...
		public String toString() {
			return "VZ_LPRC_IMAGE_INFO_Pointer{" +
					"uWidth=" + uWidth +
					", uHeight=" + uHeight +
					", uPitch=" + uPitch +
					", uPixFmt=" + uPixFmt +
	...
	public static class TH_PlateResult_Pointer extends Structure {
		...
		protected List<String> getFieldOrder() {
			return Arrays.asList(new String[]{
					"license", "color", "nColor", "nType",
					"nConfidence", "nBright", "nDirection", "rcLocation",
					"nTime", "tvPTS", "uBitsTrigType", "nCarBright", "nCarColor", "reserved0", "uId", "struBDTime", "reserved"});
		}
	}
	...
	//开闸
	int VzLPRTcp_SetIOOutputAuto(int handle, int uChnId, int nDuration);
	...
	public static interface VZLPRC_TCP_PLATE_INFO_CALLBACK extends Callback {
		public void invoke(int handle
				, Pointer pUserData
				, TH_PlateResult_Pointer.ByReference pResult
				, int uNumPlates
				, int eResultType
				, VZ_LPRC_IMAGE_INFO_Pointer.ByReference sImgFull
				, int nFullSize
				, VZ_LPRC_IMAGE_INFO_Pointer.ByReference sImgPlateClip
				, int nClipSize);
	}

	int VzLPRTcp_ImageSaveToJpeg(VZ_LPRC_IMAGE_INFO_Pointer.ByReference pImgInfo,int nImgSize, Pointer pFullPathName, int nQuality);

	// 开启透明通道
	int VzLPRTcp_SerialStart(int handle, int nSerialPort, VZDEV_TCP_SERIAL_RECV_DATA_CALLBACK func, Pointer pUserData);

	//发送数据
	int VzLPRTcp_SerialSend(int handle,int wPort, byte[] pData, int uSizeData);

	//关闭通道
	int VzLPRTcp_SerialStop(int handle);
